...
void displayWelcome() {
    ...
    // Frame title
    M5.Lcd.drawRoundRect(30, 34, 177, 67, 30, PRIMARY_COLOR);

    // Main title
    M5.Lcd.setTextSize(1.65);
    M5.Lcd.setTextColor(TEXT_COLOR, RECT_COLOR_DARK);
    ...
    // Logo
    drawWifiLogo(68, 68, 20, ACCENT_COLOR);
};
...
void displayTopBar(bool state) {
  M5.Lcd.drawRoundRect(0, 0, 148, 30, 5, PRIMARY_COLOR); // frame around status
  M5.Lcd.fillRoundRect(1, 1, 146, 28, 5, RECT_COLOR_DARK);

  M5.Lcd.setTextSize(1.18);
  M5.Lcd.setCursor(12, 10);
  M5.Lcd.setTextColor(TEXT_COLOR_ALT, RECT_COLOR_DARK);
  M5.Lcd.printf("State : ");
...
void displayVolume(uint8_t volume) {
  // Volume
  M5.Lcd.setTextSize(1.18);
  M5.Lcd.drawRoundRect(152, 0, 45, 30, 5, PRIMARY_COLOR); // frame around title
  M5.Lcd.fillRoundRect(153, 1, 43, 28, 5, RECT_COLOR_DARK); // frame around title
  M5.Lcd.setTextColor(TEXT_COLOR_ALT, RECT_COLOR_DARK);
  M5.Lcd.setCursor(158, 11);
  M5.Lcd.printf("V:");
  M5.Lcd.setTextColor(TEXT_COLOR, RECT_COLOR_DARK);
  M5.Lcd.printf("%d", volume);
  drawBatteryIndicator(202, 9);
}
...
void drawBatteryIndicator(int x, int y) {
  ...
  M5.Lcd.fillRect(x - 2, y - 2, 40, 16, BACKGROUND_COLOR);
  M5.Lcd.drawRoundRect(x, y, 16, 9, 2, color);
  M5.Lcd.fillRect(x + 16, y + 3, 2, 4, color);
  if (!unavailable) {
    const int fillWidth = (level * 12) / 100;
    if (fillWidth > 0) {
      M5.Lcd.fillRect(x + 2, y + 2, fillWidth, 5, color);
    }
  }

  M5.Lcd.setTextSize(1);
  M5.Lcd.setTextColor(color);
  M5.Lcd.setCursor(x + 20, y + 1);
  if (unavailable) {
    M5.Lcd.printf("--%%");
  } else {
...
void displayBottomBar() {
  M5.Lcd.setTextSize(1.18);
  M5.Lcd.drawRoundRect(0, 108, 240, 30, 5, PRIMARY_COLOR); // frame around controls
  ...
  M5.Lcd.drawLine(startX1, startY1 + height, startX1 + width / 2, startY1, TEXT_COLOR); // Ligne gauche
  M5.Lcd.drawLine(startX1 + width / 2, startY1, startX1 + width, startY1 + height, TEXT_COLOR); // Ligne droite
  // Dessiner la lettre "V" inversée
  M5.Lcd.drawLine(startX2, startY2, startX2 + width / 2, startY2 + height, TEXT_COLOR); // Ligne gauche inversée
  M5.Lcd.drawLine(startX2 + width / 2, startY2 + height, startX2 + width, startY2, TEXT_COLOR); // Ligne droite inversée
...
void displayWifiCount(std::vector<WiFiNetwork> networks) {
    ...
    M5.Lcd.drawRoundRect(153, 34, 86, 70, 5, PRIMARY_COLOR);
