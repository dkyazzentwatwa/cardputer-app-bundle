...
static ASIC_DATA *s_ASIC_Data = NULL;
#define ASIC_Data (*s_ASIC_Data)
...
static RGBCOLOUR *ASIC_DisplayColours = NULL;
static RGBCOLOUR *ASIC_PackedRGBColour = NULL;
static RGBCOLOUR *ASIC_PackedRGBGreyScale = NULL;
...
static unsigned char    **CartridgePages = NULL;                    /* pointer to cartridge pages */
static unsigned long	NumCartridgeBlocks;
static unsigned char	**CartridgeBlocks = NULL;

static BOOL ASIC_AllocateRuntimeState(void)
{
        if (s_ASIC_Data == NULL)
        {
                s_ASIC_Data = (ASIC_DATA *)calloc(1, sizeof(*s_ASIC_Data));
                if (s_ASIC_Data == NULL)
                {
                        return FALSE;
                }
        ...
        if (CartridgePages == NULL)
        {
                CartridgePages = (unsigned char **)calloc(32, sizeof(*CartridgePages));
                if (CartridgePages == NULL)
                {
                        free(s_ASIC_Data);
                        s_ASIC_Data = NULL;
        ...
        if (CartridgeBlocks == NULL)
        {
                CartridgeBlocks = (unsigned char **)calloc(32, sizeof(*CartridgeBlocks));
                if (CartridgeBlocks == NULL)
                {
                        free(CartridgePages);
                        CartridgePages = NULL;
...
{
                if (!ASIC_AllocateRuntimeState())
                {
                                return FALSE;
                }

		ASIC_Data.ASIC_Ram = NULL;
                ASIC_DisplayColours = NULL;
                ASIC_PackedRGBColour = NULL;
                ASIC_PackedRGBGreyScale = NULL;
		...
		ASIC_PackedRGBColour = (RGBCOLOUR *)malloc(4096 * sizeof(RGBCOLOUR));
		ASIC_PackedRGBGreyScale = (RGBCOLOUR *)malloc(4096 * sizeof(RGBCOLOUR));
        if ((ASIC_PackedRGBColour == NULL) || (ASIC_PackedRGBGreyScale == NULL))
        {
                if (ASIC_PackedRGBColour != NULL)
                {
                        free(ASIC_PackedRGBColour);
                        ASIC_PackedRGBColour = NULL;
                }
                if (ASIC_PackedRGBGreyScale != NULL)
                {
                        free(ASIC_PackedRGBGreyScale);
                        ASIC_PackedRGBGreyScale = NULL;
                }
        ...
        ASIC_DisplayColours = ASIC_PackedRGBColour;
...
void    ASIC_Finish(void)
{
        ...
        Cartridge_Remove();
                ...
                if (CartridgeBlocks!=NULL)
                {
                                free(CartridgeBlocks);
                                CartridgeBlocks = NULL;
                }
...
void    ASIC_InitCart(void)
{
		...
		if ((CartridgePages==NULL) || (CartridgeBlocks==NULL))
		{
				return;
		}
...
int             Cartridge_CalcCartSizeInBlocks(int NoOfBlocks)
{
                ...
                /* input is highest present index; convert to block count */
                blocks = NoOfBlocks + 1;
                if (blocks <= 1)
                                return 1;

                /* round up to next power-of-two, clamped to max 32 pages */
                {
                                int p2 = 1;
                                while (p2 < blocks && p2 < 32)
                                {
                                                p2 <<= 1;
                                }
                                if (p2 > 32)
...
int		Cartridge_Insert(const unsigned char *pCartridgeData, const unsigned long CartridgeDataLength)
{
        ...
        const unsigned char *pCartridge;
                ...
                if ((CartridgePages==NULL) || (CartridgeBlocks==NULL))
                {
                                return ARNOLD_STATUS_OUT_OF_MEMORY;
                }
        ...
        if (pCartridge != NULL)
        {
                ...
                /*
                 * 1) RIFF/AMS! CPR parsing (zero-copy path).
                 *    CartridgePages[] point directly into the caller ROM buffer.
                 *    CartridgeBlocks[] stay NULL -- Cartridge_Remove() must not free ROM data.
                 *    ROM buffer must remain valid for the lifetime of emulation.
                 */
                if (CartridgeLength >= 12)
                {
                        RIFF_CHUNK *pHeader = (RIFF_CHUNK *)pCartridge;
                        unsigned char *pChunkData = Riff_GetChunkDataPtr(pHeader);

                        if ((pHeader->ChunkName == RIFF_FOURCC_CODE('R','I','F','F')) &&
                                (pChunkData[0] == 'A') && (pChunkData[1] == 'M') &&
                                (pChunkData[2] == 'S') && (pChunkData[3] == '!'))
                        {
                                for (i = 0; i < 32; i++)
                                {
                                        unsigned long CartBlockChunkName = RIFF_FOURCC_CODE('c','b',((i/10)+'0'),((i%10)+'0'));
                                        RIFF_CHUNK *pChunk = Riff_FindNamedSubChunk(pHeader, CartBlockChunkName);
                                        ...
                                        if (pChunk != NULL)
                                        {
                                                ...
                                                {
                                                        CartridgePages[i] = Riff_GetChunkDataPtr(pChunk);
                                                        NumCartridgeBlocks++;
                                                        HighestBlockIndex = i;
                                                }
                                        }
                ...
                /* 2) Raw contiguous 16KB fallback (malloc needed for raw format). */
                if (NumCartridgeBlocks == 0 && (CartridgeLength % 16384) == 0)
                {
                        int RawBlocks = (int)(CartridgeLength / 16384);
                        if (RawBlocks > 32) RawBlocks = 32;
                        ...
                        for (i = 0; i < RawBlocks; i++)
                        {
                                ...
                                memcpy(CartridgeBlocks[NumCartridgeBlocks],
                                       pCartridge + ((unsigned long)i * 16384u), 16384);
...
void	ASIC_RefreshExpansionROMTable(unsigned char **pExpansionROMTable)
{
        ...
        for (i = 0; i < 256; i++)
        {
                unsigned char *pCartPage;

                pCartPage = ASIC_GetCartPage(i);
                if (pCartPage != NULL)
                {
                        pExpansionROMTable[i] = (unsigned char *)((unsigned long)pCartPage - (unsigned long)0x0c000);
                }
                else
                {
                        pExpansionROMTable[i] = NULL;
                }
        }
}
...
{
        if (ASIC_Data.ASIC_RamMask != 0)
        {
                pReadRamPtr[2] = ASIC_Data.ASIC_Ram_Adjusted;
                pReadRamPtr[3] = ASIC_Data.ASIC_Ram_Adjusted;
                pWriteRamPtr[2] = ASIC_Data.ASIC_Ram_Adjusted;
                pWriteRamPtr[3] = ASIC_Data.ASIC_Ram_Adjusted;
        }
}
...
/*                              BrightnessControl_AdjustBrightness(&SourceRGB, &DestRGB, Brightness); */
                ...
                {
		      		ASIC_DisplayColours = ASIC_PackedRGBGreyScale;
...
{        
        if ((ASIC_Data.ASIC_Ram!=NULL) && (ASIC_DisplayColours!=NULL))
    {
		int i;
