...
/* Experimental: batch CRTC_DoCycles calls to reduce call overhead. */
#define GX4_CRTC_CHUNK_BATCH 1
#define GX4_CRTC_CHUNK_NOPS  192
...
void	CPCEmulation_Run(void)
{
	...
	while (!doBreak)
	{
		...
		if (LocalNopCountToDate<=0)		
		{
			...
			if (NopsToFrameEnd!=0)
			{
#if GX4_CRTC_CHUNK_BATCH
				pendingCRTCNops += NopsToFrameEnd;
				if (pendingCRTCNops != 0)
				{
					CRTC_DoCycles(pendingCRTCNops);
					pendingCRTCNops = 0;
				}
#else
			...
			/* process and push one frame worth of audio samples */
			CPC_UpdateAudio();
...
/*			CPC_DoFrameFunc(); */
			...
			{
#if GX4_CRTC_CHUNK_BATCH
				pendingCRTCNops += -LocalNopCountToDate;
				if (pendingCRTCNops >= GX4_CRTC_CHUNK_NOPS)
				{
					CRTC_DoCycles(pendingCRTCNops);
					pendingCRTCNops = 0;
				}
