...
typedef struct CoreDispatchEntry_s
{
	uint8_t *pSFX;
	int8_t cPriority;
	int8_t cVol_Left;
...
CoreDispatchEntry_t *aDispatchQueue = NULL;
CoreDispatchEntry_t *aBatchQueue    = NULL;
CoreReqEntry_t *aReqQueue = NULL;

CoreChannel_t *aCoreChannels = NULL;
CorePatch_t   *aCorePatches  = NULL;
CoreTrack_t   *aCoreTracks   = NULL;

static void* ct_Alloc(size_t size)
{
	return malloc(size);
}

static void ct_Free(void *ptr)
{
	free(ptr);
}

static int32_t ct_EnsureAllocated(void)
{
	if(!aDispatchQueue)
	{
		aDispatchQueue = (CoreDispatchEntry_t*)ct_Alloc(
			sizeof(CoreDispatchEntry_t) * CORETONE_DISPATCH_DEPTH);
		if(!aDispatchQueue)
			return -1;
		memset(aDispatchQueue, 0,
			sizeof(CoreDispatchEntry_t) * CORETONE_DISPATCH_DEPTH);
	}

	if(!aBatchQueue)
	{
		aBatchQueue = (CoreDispatchEntry_t*)ct_Alloc(
			sizeof(CoreDispatchEntry_t) * CORETONE_DISPATCH_DEPTH);
		if(!aBatchQueue)
			return -1;
		memset(aBatchQueue, 0,
			sizeof(CoreDispatchEntry_t) * CORETONE_DISPATCH_DEPTH);
	}

	if(!aReqQueue)
	{
		aReqQueue = (CoreReqEntry_t*)ct_Alloc(
			sizeof(CoreReqEntry_t) * CORETONE_REQUEST_DEPTH);
		if(!aReqQueue)
			return -1;
		memset(aReqQueue, 0,
			sizeof(CoreReqEntry_t) * CORETONE_REQUEST_DEPTH);
	...
	if(!aCoreChannels)
	{
		aCoreChannels = (CoreChannel_t*)ct_Alloc(
			sizeof(CoreChannel_t) * CORETONE_CHANNELS);
		if(!aCoreChannels)
			return -1;
		memset(aCoreChannels, 0,
			sizeof(CoreChannel_t) * CORETONE_CHANNELS);
	}

	if(!aCorePatches)
	{
		aCorePatches = (CorePatch_t*)ct_Alloc(
			sizeof(CorePatch_t) * CORETONE_CHANNELS);
		if(!aCorePatches)
			return -1;
		memset(aCorePatches, 0,
			sizeof(CorePatch_t) * CORETONE_CHANNELS);
	}

	if(!aCoreTracks)
	{
		aCoreTracks = (CoreTrack_t*)ct_Alloc(
			sizeof(CoreTrack_t) * CORETONE_CHANNELS);
		if(!aCoreTracks)
			return -1;
		memset(aCoreTracks, 0,
			sizeof(CoreTrack_t) * CORETONE_CHANNELS);
	}
...
void ct_shutdown(void)
{
	...
	if(aCorePatches)
	{
		ct_Free(aCorePatches);
		aCorePatches = NULL;
	}

	if(aCoreChannels)
	{
		ct_Free(aCoreChannels);
		aCoreChannels = NULL;
	}
...
void ct_update(int16_t *pBuffer)
{
	...
	if(!aCoreTracks || !aCorePatches || !aCoreChannels ||
	   !aDispatchQueue || !aBatchQueue || !aReqQueue)
		return;
	...
	if(iCoreReady)
	{
		while(uiBatchIn != uiBatchOut)
		{
			...
			ct_playSFX(aBatchQueue[uiX].pSFX,
				...
				aBatchQueue[uiX].cVol_Left,
				aBatchQueue[uiX].cVol_Right);
			uiBatchOut = uiY;
		}
	}
}
