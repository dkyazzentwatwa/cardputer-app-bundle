...
const uint8_t palette_base[PALETTE_SIZE] = {
...
void palette_Load(const uint8_t* data)
{
   if(!palette_data)
   {
      palette_data = (uint8_t*)malloc(PALETTE_SIZE);
      if(!palette_data)
         return;

      /* init avec palette par défaut */
      memcpy(palette_data, palette_base, PALETTE_SIZE);
   }

   if(data)
      memcpy(palette_data, data, PALETTE_SIZE);
}