...
#define U32PTR(p) ((uint32_t *)(void *)&(p))
...
static int do_save_load(const char *file, bool save)
{
    ...
    uint32_t *snd_cycles  = hw.snd ? U32PTR(hw.snd->cycles)       : &dummy_u32;
    uint32_t *s1on        = hw.snd ? U32PTR(hw.snd->ch[0].on)     : &dummy_u32;
    uint32_t *s1p         = hw.snd ? U32PTR(hw.snd->ch[0].pos)    : &dummy_u32;
    uint32_t *s1c         = hw.snd ? U32PTR(hw.snd->ch[0].cnt)    : &dummy_u32;
    uint32_t *s1ec        = hw.snd ? U32PTR(hw.snd->ch[0].encnt)  : &dummy_u32;
    uint32_t *s1sc        = hw.snd ? U32PTR(hw.snd->ch[0].swcnt)  : &dummy_u32;
    uint32_t *s1sf        = hw.snd ? U32PTR(hw.snd->ch[0].swfreq) : &dummy_u32;
    uint32_t *s2on        = hw.snd ? U32PTR(hw.snd->ch[1].on)     : &dummy_u32;
    uint32_t *s2p         = hw.snd ? U32PTR(hw.snd->ch[1].pos)    : &dummy_u32;
    uint32_t *s2c         = hw.snd ? U32PTR(hw.snd->ch[1].cnt)    : &dummy_u32;
    uint32_t *s2ec        = hw.snd ? U32PTR(hw.snd->ch[1].encnt)  : &dummy_u32;
    uint32_t *s3on        = hw.snd ? U32PTR(hw.snd->ch[2].on)     : &dummy_u32;
    uint32_t *s3p         = hw.snd ? U32PTR(hw.snd->ch[2].pos)    : &dummy_u32;
    uint32_t *s3c         = hw.snd ? U32PTR(hw.snd->ch[2].cnt)    : &dummy_u32;
    uint32_t *s4on        = hw.snd ? U32PTR(hw.snd->ch[3].on)     : &dummy_u32;
    uint32_t *s4p         = hw.snd ? U32PTR(hw.snd->ch[3].pos)    : &dummy_u32;
    uint32_t *s4c         = hw.snd ? U32PTR(hw.snd->ch[3].cnt)    : &dummy_u32;
    uint32_t *s4ec        = hw.snd ? U32PTR(hw.snd->ch[3].encnt)  : &dummy_u32;
