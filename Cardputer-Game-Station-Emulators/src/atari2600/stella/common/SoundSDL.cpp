...
void SoundSDL::open()
{
  ...
  // Infos debug supprimées pour build embarqué léger
  // Si tu veux les garder :
  // char msg[160];
  // std::snprintf(msg, sizeof(msg),
  //   "Sound enabled: vol=%d frag=%d freq=%d channels=%d",
  //   (int)myVolume, 512, 31400, 2);
...
void SoundSDL::adjustVolume(Int8 direction)
{
  ...
  char val[32];
  std::snprintf(val, sizeof(val), "Volume set to %d", (int)percent);
  message = val;
