...
//
// SOME CONTROLLERS ARE CURRENTLY COMMENTED OUT TO SAVE FLASH SPACE
//

//
// TRY/CATCH BLOCKS ARE COMMENTED OUT TO SAVE FLASH SPACE (no fexceptions)
//
...
void Console::changeYStart(int direction)
{
  ...
  char val[16];
  std::snprintf(val, sizeof(val), "%u", (unsigned)ystart);
  myOSystem->frameBuffer().showMessage(string("YStart ") + val);
  myProperties.set(Display_YStart, val);
}
...
void Console::changeHeight(int direction)
{
  ...
  char val[16];
  std::snprintf(val, sizeof(val), "%u", (unsigned)height);
  myOSystem->frameBuffer().showMessage(string("Height ") + val);
  myProperties.set(Display_Height, val);
}
