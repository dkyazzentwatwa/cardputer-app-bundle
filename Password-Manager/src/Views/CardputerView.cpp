...
void CardputerView::topBar(const std::string& title, bool submenu, bool searchBar) {
    ...
    const int textWidth = Display->width() - batteryReservedWidth;
    ...
    if (searchBar) {
        ...
        drawSearchIcon(textWidth - 20, marginY-2, 10, PRIMARY_COLOR);
        offsetX = getCenterOffset(searchQuery, textWidth);
        Display->setCursor(offsetX, marginY);
        Display->printf(searchQuery.c_str());
    } else {
        Display->setTextColor(TEXT_COLOR);
        Display->setTextSize(TEXT_WIDE);
        auto truncatedTitle = truncateString(title, 16);
        offsetX = getCenterOffset(truncatedTitle, textWidth);
        Display->setCursor(offsetX, marginY);
        Display->printf(truncatedTitle.c_str());
    }

    drawBatteryIndicator(Display->width() - batteryReservedWidth + 5, 7);
}
...
void CardputerView::verticalSelectionSimple(
    ...
    for (size_t i = 0; i < visibleRows; ++i) {
        ...
        Display->setTextColor(isSelected ? ON_PRIMARY_COLOR : TEXT_COLOR);
...
void CardputerView::drawRect(bool selected, uint8_t margin, uint16_t startY, uint16_t sizeX, uint16_t sizeY, uint16_t stepY) {
        ...
        if (selected) {
            ...
            Display->setTextColor(ON_PRIMARY_COLOR);
        } else {
...
void CardputerView::drawBatteryIndicator(int x, int y) {
    ...
    Display->fillRect(x - 2, y - 2, 55, 16, BACKGROUND_COLOR);
    Display->drawRoundRect(x, y, 22, 11, 2, color);
    Display->fillRect(x + 22, y + 3, 2, 5, color);
    if (!unavailable) {
        const int fillWidth = (level * 18) / 100;
        if (fillWidth > 0) {
            Display->fillRect(x + 2, y + 2, fillWidth, 7, color);
        }
    }
    ...
    if (unavailable) {
        ...
        Display->printf("%d%%", static_cast<int>(level));
    }
    Display->setTextColor(TEXT_COLOR);
}
