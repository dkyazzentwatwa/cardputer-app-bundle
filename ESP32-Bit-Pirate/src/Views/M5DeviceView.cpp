...
namespace {
void drawCardputerBatteryIndicator(int x, int y) {
...
void M5DeviceView::logo() {
    ...
    M5.Lcd.drawRoundRect(8, 8, M5.Lcd.width() - 16, M5.Lcd.height() - 16, 7, PRIMARY_COLOR);
    M5.Lcd.drawRoundRect(12, 12, M5.Lcd.width() - 24, M5.Lcd.height() - 24, 5, ACCENT_COLOR);
    M5.Lcd.fillRoundRect(34, 30, M5.Lcd.width() - 68, 38, DEFAULT_ROUND_RECT, RECT_COLOR_DARK);
    M5.Lcd.drawRoundRect(34, 30, M5.Lcd.width() - 68, 38, DEFAULT_ROUND_RECT, PRIMARY_COLOR);
    M5.Lcd.setTextSize(2);
    M5.Lcd.setTextColor(PRIMARY_COLOR, RECT_COLOR_DARK);
    M5.Lcd.drawString("CYPHER", 80, 42);
...
void M5DeviceView::topBar(const std::string& title, bool submenu, bool searchBar) {
   ...
   if (x < 0) x = 0;
   drawCardputerBatteryIndicator(M5.Lcd.width() - batteryReservedWidth + 5, 7);
#else
...
void M5DeviceView::showModeName(std::string& mode, int y) {
    ...
    M5.Lcd.print("Mode: ");
    M5.Lcd.setTextColor(PRIMARY_COLOR, BACKGROUND_COLOR);
...
void M5DeviceView::drawRect(bool selected, uint8_t margin, uint16_t startY, uint16_t sizeX, uint16_t sizeY) {
    ...
    if (selected) {
       ...
       M5.Lcd.setTextColor(ON_PRIMARY_COLOR);
    } else {
