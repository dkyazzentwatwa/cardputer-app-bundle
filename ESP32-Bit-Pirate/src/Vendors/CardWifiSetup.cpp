...
namespace {
constexpr uint16_t kWifiBg = 0xffdf;
constexpr uint16_t kWifiSurface = 0xef3f;
constexpr uint16_t kWifiSurfaceStrong = 0xde7f;
constexpr uint16_t kWifiPrimary = 0x723c;
constexpr uint16_t kWifiAccent = 0x05bb;
constexpr uint16_t kWifiText = 0x18c6;
constexpr uint16_t kWifiMuted = 0x6aef;
constexpr uint16_t kWifiOnPrimary = TFT_WHITE;
}

String inputText(const String& prompt, int x, int y) {
    String data = "> ";

    M5Cardputer.Display.setTextColor(kWifiText, kWifiBg);
    ...
    while (1) {
        ...
        if (M5Cardputer.Keyboard.isChange()) {
            if (M5Cardputer.Keyboard.isPressed()) {
                ...
                M5Cardputer.Display.fillRect(0, y - 4, M5Cardputer.Display.width(), 25, kWifiBg);
...
int scanWifiNetworks() {
    ...
    while (networksCount == 0) {
        M5Cardputer.Display.fillScreen(kWifiBg);
        M5Cardputer.Display.setTextColor(kWifiText, kWifiBg);
        M5Cardputer.Display.setTextSize(1.5);
        M5.Lcd.fillRoundRect(20, 20, M5.Lcd.width() - 40, M5.Lcd.height() - 40, 5, kWifiSurface);
        M5.Lcd.drawRoundRect(20, 20, M5.Lcd.width() - 40, M5.Lcd.height() - 40, 5, kWifiPrimary);
...
static void drawWifiList(int numNetworks, int topIndex, int selectedIndex, int x, int y0, int visibleRows, int rowHeight)
{
    ...
    M5Cardputer.Display.fillRect(0, y0 - 2, screenWidth, listHeight + 4, kWifiBg);
    ...
    for (int row = 0; row < visibleRows; row++) {
        ...
        const bool selected = ssidIndex == selectedIndex;
        M5Cardputer.Display.fillRoundRect(1, y - 2, listWidth - 3, rowHeight - 2, 3, selected ? kWifiPrimary : kWifiBg);
        M5Cardputer.Display.setTextColor(selected ? kWifiOnPrimary : kWifiText, selected ? kWifiPrimary : kWifiBg);
        String line = selected ? "-> " + ssid : "   " + ssid;
    ...
    M5Cardputer.Display.fillRect(vbarX, y0 - 2, vbarWidth, listHeight + 4, kWifiBg);
    if (numNetworks > visibleRows) {
        ...
        M5Cardputer.Display.fillRect(trackX, trackY, trackWidth, trackHeight, kWifiSurfaceStrong);
        M5Cardputer.Display.drawRect(trackX, trackY, trackWidth, trackHeight, kWifiPrimary);
        ...
        M5Cardputer.Display.fillRect(trackX + 1, thumbY + 1, trackWidth - 2, thumbHeigth - 2, kWifiPrimary);
    }
}
...
String selectWifiNetwork(int numNetworks)
{
    ...
    M5Cardputer.Display.fillScreen(kWifiBg);
    M5Cardputer.Display.setTextColor(kWifiPrimary, kWifiBg);
...
bool connectToWifi(String wifiSSID, String wifiPassword) {
    ...
    M5Cardputer.Display.setTextColor(kWifiText, kWifiSurface);
    ...
    WiFi.disconnect();
    M5Cardputer.Display.setTextColor(TFT_ORANGE, kWifiBg);
